...
#define MALI_T6XX_DEFAULT_CLOCK 450000000
#else
...
#endif /* CONFIG_MALI_T6XX_DVFS_LIMIT_450 */
...
static ssize_t show_vol(struct device *dev, struct device_attribute *attr, char *buf)
{
  	...
  	ret   += snprintf(buf+  ret, PAGE_SIZE-ret, "Current operating voltage for MALI_T6XX = %d", vol);
...
#else /* CONFIG_MALI_T6XX_DVFS_LIMIT_450 */
...
#ifndef CONFIG_MALI_T6XX_DVFS_LIMIT_450
  	else if (sysfs_streq("533", buf)) {
...
/** The sysfs file @c clock, fbdev.
  *
  * This is used for obtaining information about the MALI_T6XX operating clock & framebuffer address,
  */
...
mali_error kbase_platform_init(struct kbase_device *kbdev)
{
  	...
  	{
  		/* EXYNOS5 DT data does not specify supply info for G3D regulator so no error reporting here */
