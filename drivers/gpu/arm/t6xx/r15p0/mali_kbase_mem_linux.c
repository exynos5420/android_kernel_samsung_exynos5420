...
static struct kbase_va_region *kbase_mem_from_umm(struct kbase_context *kctx, int fd, u64 *va_pages, u64 *flags)
{
	...
	*flags &= ~BASE_MEM_SAME_VA;
	...
	if (*flags & BASE_MEM_SECURE) {
		/* MALI_SEC_SECURE_RENDERING */
		if (kctx->kbdev->protected_mode_support == true &&
			kctx->kbdev->protected_ops != NULL) {
			int err = -EINVAL;
			err = kctx->kbdev->protected_ops->secure_mem_enable(kctx->kbdev, fd, *flags, reg);
			if (err)
		...
		u64 new_pages, u64 old_pages)
{
	...
	unmap_mapping_range(kctx->filp->f_mapping->host->i_mapping,
