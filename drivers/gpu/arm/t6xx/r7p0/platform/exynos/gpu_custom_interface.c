...
static int gpu_get_asv_table(struct exynos_context *platform, char *buf, size_t buf_size)
{
	...
	for (i = gpu_dvfs_get_level(platform->gpu_max_clock_limit); i <= gpu_dvfs_get_level(platform->gpu_min_clock_limit); i++) {
...
static ssize_t show_time_in_state(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = gpu_dvfs_get_level(platform->gpu_min_clock_limit); i >= gpu_dvfs_get_level(platform->gpu_max_clock_limit); i--) {
