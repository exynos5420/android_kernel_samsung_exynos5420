...
#else
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "asv table clk[%d to %d]MHz, vol[%d (margin : %d) real: %d]mV\n",
...
static int pm_callback_runtime_on(struct kbase_device *kbdev)
{
	...
	if (IS_ERR(mout_vpll)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "failed to clk_get [mout_vpll]\n");
		return 0;
	}
	aclk_g3d_dout = clk_get(dev, "aclk_g3d_dout");
	if (IS_ERR(aclk_g3d_dout)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "failed to clk_get [aclk_g3d_dout]\n");
		return 0;
	}
	aclk_g3d_sw = clk_get(dev, "aclk_g3d_sw");
	if (IS_ERR(aclk_g3d_sw)) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "failed to clk_get [aclk_g3d_sw]\n");
		return 0;
	}

	ret = clk_set_parent(aclk_g3d, aclk_g3d_sw);//aclk_g3d
	if (ret < 0) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "failed to clk_set_parent [platform->aclk_g3d]\n");
		return 0;
	}
	ret = clk_set_parent(aclk_g3d_sw, aclk_g3d_dout);
	if (ret < 0) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "failed to clk_set_parent [aclk_g3d_sw]\n");
		return 0;
	}
	ret = clk_set_parent(aclk_g3d_dout, mout_vpll);
	if (ret < 0) {
		GPU_LOG(DVFS_ERROR, DUMMY, 0u, 0u, "failed to clk_set_parent [aclk_g3d_dout]\n");
		return 0;
	}
