...
DEVICE_ATTR(gpu_thermal1, 0644, gpu_thermal1_show, gpu_thermal1_write);
DEVICE_ATTR(gpu_thermal2, 0644, gpu_thermal2_show, gpu_thermal2_write);
DEVICE_ATTR(gpu_thermal3, 0644, gpu_thermal3_show, gpu_thermal3_write);
DEVICE_ATTR(gpu_thermal4, 0644, gpu_thermal4_show, gpu_thermal4_write);
DEVICE_ATTR(gpu_thermal_tripping, 0444, gpu_thermal_tripping, NULL);
DEVICE_ATTR(gpu_volt_table, 0644, gpu_volt_table_show, gpu_volt_table_write);
...
int gpu_sysfs_create_sysfs_files(struct device *dev)
{
	...
	if (device_create_file(dev, &dev_attr_gpu_thermal1)) {
		pr_info("GPU_SYSFS: Couldn't create sysfs file %d\n", __LINE__);
		goto out;
	}

	if (device_create_file(dev, &dev_attr_gpu_thermal2)) {
		pr_info("GPU_SYSFS: Couldn't create sysfs file %d\n", __LINE__);
		goto out;
	}

	if (device_create_file(dev, &dev_attr_gpu_thermal3)) {
		pr_info("GPU_SYSFS: Couldn't create sysfs file %d\n", __LINE__);
		goto out;
	}

	if (device_create_file(dev, &dev_attr_gpu_thermal4)) {
		pr_info("GPU_SYSFS: Couldn't create sysfs file %d\n", __LINE__);
		goto out;
	}

	if (device_create_file(dev, &dev_attr_gpu_thermal_tripping)) {
		pr_info("GPU_SYSFS: Couldn't create sysfs file %d\n", __LINE__);
		goto out;
	}

	if (device_create_file(dev, &dev_attr_gpu_volt_table)) {
		pr_info("GPU_SYSFS: Couldn't create sysfs file %d\n", __LINE__);
		goto out;
	}
	return 0;
...
void gpu_sysfs_remove_sysfs_files(struct device *dev)
{
	...
	device_remove_file(dev, &dev_attr_gpu_thermal1);
	device_remove_file(dev, &dev_attr_gpu_thermal2);
	device_remove_file(dev, &dev_attr_gpu_thermal3);
	device_remove_file(dev, &dev_attr_gpu_thermal4);
	device_remove_file(dev, &dev_attr_gpu_thermal_tripping);
	device_remove_file(dev, &dev_attr_gpu_volt_table);
}
