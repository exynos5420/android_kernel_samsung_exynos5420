		...
		struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	/* Derive core number */
	core_num = ASCII_TO_DIGIT_NUM(buf[0]);
	if ((core_num >= 0) && (core_num < COUNT_OF_CORES))		/* From core 0 to core 3 */
		new_core = core_num;
	else {
		pr_err("Enter correct core number(0~3)\n");
		return count;
	}
		...
		struct kobj_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "Secure OS is on core [%d]\n", current_core);
}
