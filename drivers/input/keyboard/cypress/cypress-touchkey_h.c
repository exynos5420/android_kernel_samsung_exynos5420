...
extern int tc380_fw_update(struct touchkey_i2c *data);
...
static void touchkey_set_dvfs_lock(struct touchkey_i2c *tkey_i2c,
					uint32_t on)
{
	if (TKEY_BOOSTER_DISABLE == tkey_i2c->boost_level || touchkey_set_dvfs_lock == 0)
		return;
...
{
	/* TC380 */
	if (tkey_i2c->ic_type == TKEY_IC_TYPE_TC380) {
		tkey_i2c->fw_update = tc380_fw_update;
		tk_fw_name = FW_PATH_TC380;
		md_ver_file = 0;
		printk(KERN_DEBUG"touchkey:change fw path to tc380\n");
		return ;
	}
...
int tkey_load_fw_built_in(struct touchkey_i2c *tkey_i2c)
{
	...
	if (tkey_i2c->ic_type == TKEY_IC_TYPE_TC380) {
		...
		fw_ver_file = tkey_i2c->fw_img->first_fw_ver;
	} else
		firmware_data = (u8 *)tkey_i2c->firm_data->data;
...
int tkey_load_fw_sdcard(struct touchkey_i2c *tkey_i2c)
{
	...
	if (tkey_i2c->ic_type) {
		ums_data = &firmware_data;
		fw_path = TKEY_FW_PATH;
	} else {
		ums_data = (u8 **)&tkey_i2c->fw_img;
		fw_path = TKEY_TC380_FW_PATH;
	}
...
#endif
			goto run_fw_update;
	}else if (is_same_module_class(tkey_i2c)) {
...
static void touchkey_i2c_update_work(struct work_struct *work)
{
	...
	while (retry--) {
		ret = tkey_i2c->fw_update(tkey_i2c);
		if (ret != 0) {
			msleep(50);
 ...
 end_fw_update:
#if defined(TK_HAS_AUTOCAL)
	if (tkey_i2c->ic_type == TKEY_IC_TYPE_CYPRESS)
	...
	const struct i2c_device_id *id)
{
	...
	tkey_i2c->ic_type = tkey_i2c->pdata->check_ic_type();
	dev_info(&client->dev, "IC Type %s\n", tkey_i2c->ic_type ? "cypress" : "tc380");
