...
#ifdef USE_TOUCH_KEYARRAY_T15
static void mxt_treat_T15_object(struct mxt_data *data, u8 *msg)
{
	/* whether reportid is thing of TOUCH_KEYARRAY */
	...
	if (msg[1] & DETECT_MSG_MASK) {
		...
		if (tsp_keystatus != TOUCH_KEY_NULL) {
			...
			case TOUCH_2KEY_MENU:
				input_report_key(data->input_dev,
					KEY_MENU, MXT_STATE_RELEASE);
				break;
			case TOUCH_2KEY_BACK:
				input_report_key(data->input_dev,
					KEY_BACK, MXT_STATE_RELEASE);
				break;
			default:
				break;
			}
...
#else
			dev_info(&data->client->dev, "[TSP_KEY] R\n");
#endif
		...
		case TOUCH_2KEY_MENU:
			input_report_key(data->input_dev,
				KEY_MENU, MXT_STATE_PRESS);
			tsp_keystatus = TOUCH_2KEY_MENU;
			break;
		case TOUCH_2KEY_BACK:
			input_report_key(data->input_dev,
				KEY_BACK, MXT_STATE_PRESS);
		...
		tsp_2keyname[tsp_keystatus - 1]);
#else
		dev_info(&data->client->dev, "[TSP_KEY] P\n");
#endif
	} else {
		switch (tsp_keystatus) {
		case TOUCH_2KEY_MENU:
...
static void mxt_handle_init_data(struct mxt_data *data)
{
	...
	dev_info(&data->client->dev, "Use touchkey for GFF\n");
	mxt_write_object(data, TOUCH_KEYARRAY_T15,
	T15_CTRL, 131);
	mxt_write_object(data, TOUCH_KEYARRAY_T15,
	T15_XORIGIN, 26);
#else
...
static int mxt_flash_fw_on_probe(struct mxt_fw_info *fw_info)
{
	...
	dev_info(dev, "Rev04 does not need to update tsp firmware\n");
	goto out;
#endif
