...
static int max77803_read_word(struct i2c_client *i2c, u8 reg)
{
	struct max77803_dev *max77803 = i2c_get_clientdata(i2c);
	int ret;
			      ...
			      const struct i2c_device_id *id)
{
	...
	max77803->test = i2c_new_dummy(i2c->adapter, I2C_ADDR_TEST);
	i2c_set_clientdata(max77803->test, max77803);

	// Start Over-write wrong-Trimmed bit //

	// 1. Test Register Access Enabled
	max77803_write_reg(max77803->i2c, 0xFE, 0xC5);
	// 2. Enable TST_KEY
	max77803_write_reg(max77803->test, 0xB3, 0x0C);
	// 3. Read 0x2E with word unit.
	reg_data16 = max77803_read_word(max77803->test, 0x2E);
	...
	if ((reg_data16 & 0x2000) == 0) {
		...
		for (i = 0x21; i <= 0x2A; i++) {
			...
			reg_data16 = max77803_read_word(max77803->test, i);
		...
		for (i = 0x21; i <= 0x2A; i++) {
			...
			max77803_write_reg(max77803->test, i, str_data[i-0x21]);
		}
		// 7. Write 0x2E
		max77803_write_reg(max77803->test, 0x2E, (reg_data | 0x20));

		// 8. Write 0x20 to 0x40.
		max77803_write_reg(max77803->test, 0x20, 0x40);
	}

	// 9. Disable TST_KEY, Write 0xB3 to 0x00
	max77803_write_reg(max77803->test, 0xB3, 0x00);

	// 10. Test Register Access Disabled, Write 0xFE to 0x00
	max77803_write_reg(max77803->i2c, 0xFE, 0x00);
