...
static void mdm_peripheral_connect(struct mdm_modem_drv *mdm_drv)
{
	 ...
	 */
	if (mdm_drv->pdata->peripheral_platform_device_ehci)
		platform_device_add(mdm_drv->pdata->peripheral_platform_device_ehci);
	...
	if (mdm_drv->pdata->peripheral_platform_device_ohci)
		platform_device_add(mdm_drv->pdata->peripheral_platform_device_ohci);
...
static void mdm_peripheral_disconnect(struct mdm_modem_drv *mdm_drv)
{
	 ...
	 */
	if (mdm_drv->pdata->peripheral_platform_device_ohci)
		platform_device_del(mdm_drv->pdata->peripheral_platform_device_ohci);
	if (mdm_drv->pdata->peripheral_platform_device_ehci)
		platform_device_del(mdm_drv->pdata->peripheral_platform_device_ehci);
	pr_info("%s done\n", __func__);
...
static void mdm_power_down_common(struct mdm_modem_drv *mdm_drv)
{
	...
	/* during AP power_down, skb was free somewhere.
	 * and delete this disconnection logic temporarily. */
	/* mdm_peripheral_disconnect(mdm_drv); */

	/* Assert the soft reset line whether mdm2ap_status went low or not */
	gpio_direction_output(MDM_GPIO(AP2MDM_SOFT_RESET),
					soft_reset_direction);
	pr_info("%s:id %d: MDM2AP_STATUS never went low. Doing a hard reset\n",
...
static void mdm_do_first_power_on(struct mdm_modem_drv *mdm_drv)
{
	...
	pr_info("%s:id %d: Powering on modem for the first time\n",
		   __func__, mdm_drv->device_id);
	mdm_peripheral_disconnect(mdm_drv);
...
static void mdm_do_soft_power_on(struct mdm_modem_drv *mdm_drv)
{
	...
	pr_info("%s: id %d:  soft resetting mdm modem\n",
		   __func__, mdm_drv->device_id);
	mdm_peripheral_disconnect(mdm_drv);
	mdm_toggle_soft_reset(mdm_drv);
