...
struct mdm_device {
	...
	struct delayed_work mdm_reconnect_work;
...
void call_mdm_start_ssr(void)
{
	struct mdm_device *mdev;
	unsigned long flags;

	spin_lock_irqsave(&mdm_devices_lock, flags);
	mdev = list_entry(mdm_devices.next, struct mdm_device, link);
	if (mdev) {
		mdm_start_ssr(mdev);
	}
	spin_unlock_irqrestore(&mdm_devices_lock, flags);
...
static unsigned char *mdm_read_err_report(void)
{
	...
	set_fs(KERNEL_DS);
	do {
		filp = filp_open("/data/cp_log/mdm_err.log", \
				...
				unsigned long arg)
{
	...
	pr_info("%s: Entering ioctl cmd = %d, mdm id = %d\n",
	...
	case NORMAL_BOOT_DONE:
		pr_info("%s: check if mdm id %d is booted up\n",
				 __func__, mdev->mdm_data.device_id);
		get_user(status, (unsigned long __user *) arg);
		if (status) {
			pr_info("%s: normal boot of mdm id %d failed\n",
					 __func__, mdev->mdm_data.device_id);
	...
	case GET_FORCE_RAMDUMP:
		get_user(status, (unsigned long __user *) arg);
		pr_info("%s: mdm got message from RIL : %s\n",
				__func__, status ? "Crash" : "Silent Reset");
		if (status)
			mdm_start_ssr(mdev);
#if 0
		/* To do : silent reset function */
		else
			mdm_silent_reset();
#endif
		break;
...
static void mdm_status_fn(struct work_struct *work)
{
	...
	if (value) {
		...
		schedule_delayed_work(&mdev->mdm_reconnect_work,
...
static int mdm_subsys_powerup(const struct subsys_desc *crashed_subsys)
{
	...
	/* ipc driver don't control ehci connect/disconnect function anymore.
	 * kickstart control ehci device using sysfs.
	 * so this code have chance to cause crash. */
	/* mdm_ops->power_on_mdm_cb(mdm_drv); */
				...
				struct mdm_device *mdev)
{
	struct mdm_modem_drv *mdm_drv = &mdev->mdm_data;
	int i, val, rc = 0;
//	enum of_gpio_flags flags = OF_GPIO_ACTIVE_HIGH;
	...
	val = of_get_named_gpio(node, "qcom,ap2mdm-soft-reset-gpio", 0);
	if (val >= 0) {
...
#if !defined(CONFIG_ARCH_EXYNOS)
	val = of_get_named_gpio(node, "qcom,ap2mdm-kpdpwr-gpio",0);
	if (val >= 0) {
			...
			struct mdm_device *mdev)
{
	...
	/* MDM2AP_ERRFATAL */
	pres = platform_get_resource_byname(pdev, IORESOURCE_IRQ,
							"MDM2AP_ERRFATAL");
...
static int mdm_configure_ipc(struct mdm_device *mdev)
{
	...
	/* ERR_FATAL irq. */
	irq = gpio_to_irq(MDM_GPIO(MDM2AP_ERRFATAL));
	if (irq < 0) {
...
errfatal_err:
	 /* status irq */
	irq = gpio_to_irq(MDM_GPIO(MDM2AP_STATUS));
	if (irq < 0) {
...
status_err:
	if (GPIO_IS_VALID(MDM_GPIO(MDM2AP_PBLRDY))) {
		irq = gpio_to_irq(MDM_GPIO(MDM2AP_PBLRDY));
		if (irq < 0) {
...
pblrdy_err:
	 ...
	 */
	if (GPIO_IS_VALID(MDM_GPIO(AP2MDM_PMIC_PWR_EN))) {
		pr_info("%s: pulling ap2mdm_pmic_pwr_en_gpio high\n",
				 __func__);
...
static int mdm_modem_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		...
		INIT_DELAYED_WORK(&mdev->mdm_reconnect_work, mdm_reconnect_fn);
