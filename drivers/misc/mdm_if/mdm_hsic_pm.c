...
void request_active_lock_set(void)
{
	struct mdm_hsic_pm_data *pm_data =
				get_pm_data_by_dev_name("mdm_hsic_pm0");
	pr_info("%s\n", __func__);
...
void request_active_lock_release(void)
{
	struct mdm_hsic_pm_data *pm_data =
				get_pm_data_by_dev_name("mdm_hsic_pm0");
	pr_info("%s\n", __func__);
...
void set_host_stat(enum pwr_stat status)
{
	struct mdm_hsic_pm_data *pm_data =
				get_pm_data_by_dev_name("mdm_hsic_pm0");

	if (!pm_data) {
		pr_info("%s: there is no pm_data\n", __func__);
		return;
	}
 ...
 */
int check_udev_suspend_allowed(void)
{
	struct mdm_hsic_pm_data *pm_data =
				get_pm_data_by_dev_name("mdm_hsic_pm0");
	struct device *dev;
...
static int mdm_pm_parse_dt_pdata(struct device *dev, struct mdm_hsic_pm_data *pm_data)
{
	int ret;

	ret = parse_dt_gpio_pdata(dev->of_node, pm_data);
	if (ret < 0) {
...
{
	int ret;
	struct mdm_hsic_pm_data *pm_data = NULL;
	...
	if (pdev->dev.of_node) {
		ret = mdm_pm_parse_dt_pdata(&pdev->dev, pm_data);
		if (ret < 0) {
		...
		if (ret < 0) {
			pr_err("%s: gpio initialization failed!\n", __func__);
			goto err_gpio_init_fail;
		}
#if 0
		if (!pm_data) {
...
#else
	register_usb2phy_notifier(&pm_data->phy_nfb);
#endif
