...
static int suspend_check_cp_hostwakeup(struct link_device *ld)
{
	struct usb_link_device *usb_ld = to_usb_link_device(ld);
	int ret;
	...
	ret = get_hostwake(usb_ld->link_pm_data) ? -EAGAIN : 0;
	mif_info("ret = %d\n", ret);
	return ret;
}
...
static struct  link_usb_id {
	int vid;
	int pid;
} link_device_ids[] = {
	{0x1519, 0x0443}, /* XMM6360 */
	{0x1519, 0x0020}, /* XMM6262 */
};
...
static bool linkpm_known_device(const struct usb_device_descriptor *desc)
{
	int i;
...
struct link_device *hsic_create_link_device(void *data)
{
	...
	init_completion(&ld->raw_tx_resumed_by_cp);
	ld->suspend_check_cpwake = suspend_check_cp_hostwakeup;
