...
/* Fixed modem unknown issue (Kona 3G) */
		...
		mif_err("sim state changed. (iod: %s, state: [online=%d, changed=%d])\n",
			...
			size_t count, loff_t *ppos)
{
	...
	if (err != tx_size)
		mif_err("WARNNING: wrong tx size: %s, format=%d count=%d, tx_size=%d, return_size=%d",
