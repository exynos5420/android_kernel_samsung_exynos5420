...
#define MMDOCK_PSY_CHG_NAME "sec-charger"
...
{
	struct power_supply *psy = power_supply_get_by_name(MMDOCK_PSY_CHG_NAME);
	union power_supply_propval value;

	if (!psy) {
		pr_err("%s: fail to get %s psy\n", __func__, MMDOCK_PSY_CHG_NAME);
		return;
	}
	if (host_state)
...
static int call_battery_notify(struct usb_device *dev, bool bOnOff)
{
	...
	if (bOnOff) {
		if (count == 1) {
			if (cable_type == CABLE_TYPE_SMARTDOCK_TA_MUIC ||
				cable_type == CABLE_TYPE_MMDOCK_MUIC) {
