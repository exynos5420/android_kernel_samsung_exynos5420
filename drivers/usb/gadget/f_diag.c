...
static void diag_function_disable(struct usb_function *f)
{
	...
	if (dev->ch.notify) {
		if (dev->qxdm_ops)
			dev->ch.notify(dev->ch.priv,
					USB_DIAG_QXDM_DISCONNECT, NULL);
		else
			dev->ch.notify(dev->ch.priv,
					USB_DIAG_DISCONNECT, NULL);
		dev->qxdm_ops = 0;
	}
		...
		unsigned intf, unsigned alt)
{
	...
	ret = config_ep_by_speed(cdev->gadget, f, dev->in);
	if (ret) {
		dev->in->desc = NULL;
	...
	if (dev->out->driver_data)
		usb_ep_disable(dev->out);
	dev->out->driver_data = dev;
	ret = config_ep_by_speed(cdev->gadget, f, dev->out);
	if (ret) {
		dev->out->desc = NULL;
		ERROR(cdev, "config_ep_by_speed failes for ep %s, result %d\n",
			dev->out->name, ret);
